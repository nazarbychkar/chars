  ...
  useEffect(() => {
    async function fetchData() {
      ...
      for (const order of orders as DashboardOrder[]) {
        ...
        if (currency === "EUR") {
          // рахуємо окремо, але зараз сумуємо все разом для спрощення
          totalRev += orderTotal;
        } else {
          totalRev += orderTotal;
        }
        ...
        if (orderDetails.payment_type === "prepay") {
          const prepayValue = currency === "EUR" ? 300 / 40 : 300; // TODO: зафіксувати реальну передплату в EUR окремо
          prepaid += prepayValue;
          remaining += Math.max(0, orderTotal - prepayValue);
